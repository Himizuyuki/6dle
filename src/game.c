...
* from the secret word find the letters that are : (take care if there are
double letters or more)
*                                                  -in the right place (green)
                                                  -in the word but at the wrong
place (yellow)
*                                                  -not in the word
* Game loop :
*  -> Initialize the Game
...
*  :> while playing :
*     -> while the word has not been found or the nb guesses is different than
max Guesses
*         .> Ask for user input to build a six letter word
*             |> check if the word is long enough and if so check if it belongs
to the WB
*             |> if not => Ask for user input again
*         .> update game state => array of guessed words + validate the word +
nb guesses
*         .> (Terminal Version) Pretty print the results with color changes in
the terminal
...
void printLine(Game *game, int index)
{
    ...
    {
        printf("║ ");
        if (game->colorWords[index][i] == 'g')
            green();
        else if (game->colorWords[index][i] == 'y')
            yellow();
        else if (game->colorWords[index][i] == 'r')
            red();
...
char isDuplicate(Game *game)
{
    ...
    {
        if (strcmp(game->guessedWords[i], game->guessedWords[game->nb_Guesses])
            == 0)
            return 1;
    }
    return 0;
